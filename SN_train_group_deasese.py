...
from Siamese_Network import SiameseNetwork, Similarity_Loss_grouped  
    ...
    for epoch in range(epochs):
        ...
        for start_idx in range(0, total_rows, batch_size):
            ...
            images = load_images_from_folder(
                image_folder, start_idx, len(batch), transform
            ).to(device)
            text_input = texts
...
if __name__ == "__main__":
    csv_path_train = "gruop_deasease/reports_with_groups.csv"
    csv_path_val = "gruop_deasease/val_reports_with_groups.csv"
    image_folder = "train_mimic"
    ...
    loss_fn = Similarity_Loss_grouped(temperature=0.1).to(device)
