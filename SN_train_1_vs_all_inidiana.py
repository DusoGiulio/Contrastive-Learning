...
def load_images_from_folder(folder_path, image_names, start_idx, batch_size, transform=None):
    images = []
    for i in range(start_idx, min(start_idx + batch_size, len(image_names))):
...
def train(model, df_train, image_folder, loss_fn, optimizer, batch_size, epochs, save_folder="1_vs_all", device='cuda'):
    ...
    for epoch in range(epochs):
        ...
        for start_idx in range(0, total_rows, batch_size):
            ...
            image_names = batch["image_name"].tolist()
            texts = batch["findings"].tolist()

            images = load_images_from_folder(image_folder, image_names, start_idx % batch_size, len(batch), transform).to(device)
            text_input = texts
            ...
            positive_pairs = [(i, i) for i in range(len(batch))] 
...
if __name__ == '__main__':
    csv_path = r"data\indiana\indiana\dataset_train.csv"
    image_folder = r"data\indiana\indiana\Images\Images"
    save_folder = "1_vs_all\indiana_model"
    ...
    train(model, df_train, image_folder, loss_fn, optimizer, batch_size=100, epochs=10, save_folder=save_folder, device=device)
